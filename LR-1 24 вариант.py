...
for i in range(len(test_data)): 
    if check_hex(test_data[i]): #Проверяем является ли слово 16 числом <1024
        try:  
            ...
            t = test_data[i] 
        if int(test_data[i], 16) <= int(t, 16): #Если проверяемое число меньше или равно последнему элементу в последовательности, то последовательность - убывающая и данное число добавляется в конец списка. 
